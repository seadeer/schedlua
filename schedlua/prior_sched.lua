...
local PQueue = require("schedlua.pqueue")
...
function PScheduler.init(self, ...)
	--print("==== PScheduler.init ====")
	local obj = {
		TasksReadyToRun = PQueue();
	}
	setmetatable(obj, PScheduler_mt)
	
	return obj;
end

function PScheduler.new(self, ...)
	return self:init(...)
end
...
function PScheduler.tasksPending(self)
...
function PScheduler.scheduleTask(self, task, priority, params)
	--print("PScheduler.scheduleTask: ", task, params)
	params = params or {}
...
function PScheduler.removeFiber(self, fiber)
	--print("REMOVING DEAD FIBER: ", fiber);
	return true;
end

function PScheduler.inMainFiber(self)
	return coroutine.running() == nil; 
end

function PScheduler.getCurrentTask(self)
	return self.CurrentFiber;
end

function PScheduler.suspendCurrentFiber(self, ...)
	self.CurrentFiber.state = "suspended"
end

function PScheduler.step(self)
	-- Now check the regular fibers
	local task = self.TasksReadyToRun:dequeue()

	-- If no fiber in ready queue, then just return
	if task == nil then
		--print("PScheduler.step: NO TASK")
		return true
	end
...
function PScheduler.yield(self, ...)
